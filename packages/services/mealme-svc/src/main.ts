...
const logger = new Logger('Halen Mealme Service');
let cachedServer: Server;

if (
  process.env.ENVIRONMENT === 'local' &&
  process.env.BOOTSTRAP_NEST_APP === 'true'
) {
  nestBootstrap();
} else {
...
export function setupSwagger(app: INestApplication) {
  const options = new DocumentBuilder()
    .setTitle('Halen Technologies - MealMe Service')
    .setDescription('MealMe Service for Halen Technologies')
    .setVersion('1.0')
    .addTag('')
    .addBearerAuth()
    .build();
